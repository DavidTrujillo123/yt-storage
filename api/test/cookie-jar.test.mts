...
const browserExport = Buffer.from(
  [
    ...
    '',
    line('.youtube.com', '__Secure-1PSID', 'session', true),
...
describe('filterCookieJar', () => {
  ...
  it('refuses a jar holding only cross-site cookies', () => {
    // What a header copied from an embedded player looks like: the 3P names
    // plus the player's own ST-* cookies, and nothing YouTube treats as a
    // signed-in request. It used to store cleanly and then report every
    // private video as private, which sends people to look at the video.
    const embedded = Buffer.from(
      [
        line('.youtube.com', '__Secure-3PSID', 'g.a000', true),
        line('.youtube.com', '__Secure-3PAPISID', 'x', true),
        line('.youtube.com', 'ST-xuwub9'),
        line('.youtube.com', 'VISITOR_INFO1_LIVE'),
      ].join('\n'),
      'utf8',
    );
    assert.throws(() => filterCookieJar(embedded), /third-party cookies only/);
  });
});

describe('hasSessionCookie', () => {
  it('accepts a first-party session cookie', () => {
    for (const name of ['__Secure-1PSID', 'SID']) {
      assert.equal(hasSessionCookie(Buffer.from(line('.google.com', name))), true);
    }
  });

  it('rejects the cross-site names, which authenticate nothing', () => {
    for (const name of ['__Secure-3PSID', '__Secure-3PAPISID', 'SAPISID']) {
      assert.equal(hasSessionCookie(Buffer.from(line('.google.com', name))), false);
    }
  });

  it('sees a session cookie behind the HttpOnly prefix', () => {
    assert.equal(
      hasSessionCookie(Buffer.from(line('.youtube.com', '__Secure-1PSID', 'v', true))),
      true,
    );
  });
...
describe('jarFromHeader', () => {
  ...
  const header =
    'VISITOR_INFO1_LIVE=abc; __Secure-1PSID=g.a000xyz; SAPISID=sapi/value; ' +
...
describe('cookieHeaderFromPaste', () => {
  const cookies = "__Secure-1PSID=g.a000xyz; SAPISID=sapi/value; PREF=tz=Europe.Madrid";
